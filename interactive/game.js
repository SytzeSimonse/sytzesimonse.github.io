...
class SceneManager {
    ...
    setupEventListeners() {
        // Scene 0 → Scene 1: Start button click
        const startGameBtn = document.getElementById('start-game-btn');
        if (startGameBtn) {
            startGameBtn.addEventListener('click', () => {
                console.log('🎮 Starting game...');
                this.showScene('scene-whatsapp');
                // Animate WhatsApp notification appearing after a short delay
                setTimeout(() => {
                    const notification = document.getElementById('whatsapp-notification');
                    if (notification) {
                        notification.style.opacity = '1';
                        notification.style.pointerEvents = 'auto';
                        notification.style.animation = 'slideInBounce 0.8s cubic-bezier(0.68, -0.55, 0.265, 1.55)';
                    }
                }, 500);
            });
        }
